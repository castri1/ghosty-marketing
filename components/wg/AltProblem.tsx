...
const ICONS = [PX.laptop, PX.scatter, PX.question];

export function AltProblem({ dict }: { dict: Dictionary["home"]["problem"] }) {
  return (
      ...
      <div className="mx-auto max-w-6xl px-5 py-24 sm:px-8 md:py-32">
        <Rise className="max-w-3xl">
          ...
          <h2 className="mt-5 text-balance font-display text-4xl font-semibold tracking-[-0.02em] sm:text-5xl md:text-6xl">
            {dict.titleLead}{" "}
            <em className="text-muted">{dict.titleEm}</em>
          </h2>
        ...
        <div className="mt-16 grid gap-5 md:grid-cols-3">
          {dict.pains.map((pain, i) => (
            <Rise key={pain.title} delay={i * 0.1}>
              <article className="group h-full border border-line bg-mist p-8 transition-all duration-200 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[8px_8px_0_0_#D4DCD6]">
                <PixelIcon
                  rows={[...ICONS[i]]}
